...
public class SandhataDaoImpl implements SandhataDao {
    ...
    @Override
    public void getMyTimeSheet(List<TimeSheetDto> timeSheetDtoList, String companyName, String department, String projectName, String name, Date startDate, Date endDate) {
        try {
            Session session = factory.getCurrentSession();
            session.beginTransaction();
            Query query=session.createQuery(" from TimeSheetEntity where companyName=:companyName and department=:department and projectName=:projectName and name=:name and date>=:startDate and date <=:endDate");
            query.setString("companyName", companyName);
            query.setString("department", department);
            query.setString("projectName", projectName);
            ...
            List<TimeSheetEntity> timeSheetEntityList=query.list();
            for(TimeSheetEntity temp:timeSheetEntityList){
                TimeSheetDto timeSheetDto=new TimeSheetDto(temp);
                timeSheetDtoList.add(timeSheetDto);
                System.out.println(timeSheetDto.toString());
            }
            session.getTransaction().commit();
        } catch (Exception exc) {
            System.out.println("Caught in Exception inside Class :\tSandhataDaoImpl\tMethod :\tGetMyTimeSheet\nAnd the Exception is :\t" + exc);
            exc.printStackTrace();
        }
    }
